...
namespace WebAlumnosFP
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        protected void btnBorrar_Click(object sender, EventArgs e)
        {
            ...
            // Capturamos el idAlumnos del registro seleccionado del dgv 
            //                                      fila dgv    celda del idAlumno
            int idAlumno = Convert.ToInt32(dgv.Rows[dgv.SelectedIndex].Cells[1].Text);

            LNyAD.EliminarAlumno(idAlumno);

            dgv.SelectedIndex = -1; // <-- Hacemos que no haya ningún registro seleccionado después de borrar
        ...
        void MostrarConfirmacion(bool loMostramos)
        {
            if(loMostramos)
            {
                dgv.Enabled = false;        // <-- Deshabilitamos el GridView
                ddlGrupos.Visible = false;
                btnAnyadir.Visible = false;
                btnGestionGrupos.Visible = false;
                lbConfirmacion.Text = "¿Eliminamos a " + dgv.SelectedRow.Cells[4].Text + "?";
                btnBorrar.Visible = true;
                btnNoBorrar.Visible = true;
                lbConfirmacion.Visible = true;
                panelControles.BackColor = System.Drawing.Color.Red;
            }
            else
            {
                dgv.Enabled = true;        // <-- Habilitamos el GridView
                ddlGrupos.Visible = true;
                btnAnyadir.Visible = true;
                btnGestionGrupos.Visible = true;
                btnBorrar.Visible = false;
                btnNoBorrar.Visible = false;
                lbConfirmacion.Visible = false;
                panelControles.BackColor = System.Drawing.Color.Orange;
            }
        }
